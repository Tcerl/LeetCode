# 🌳 Career-Mastery — Cây Kiến Thức Sự Nghiệp & Phỏng Vấn (Senior Companion)

> **Mục đích:** Lớp kiến thức bổ sung cho [`01-Roadmaps/`](../../01-Roadmaps/), [`04-Interview-Prep/`](../../04-Interview-Prep/) và [`interview_prep/`](../../interview_prep/). Các thư mục đó đã có lộ trình chi tiết và ngân hàng câu hỏi phong phú theo từng công nghệ. Cây này là **lớp tổng hợp** — nơi kiến thức kỹ thuật từ `DSA-Mastery`, `Backend-Mastery`, `Cloud-DevOps-Mastery` được chuyển hóa thành **cách trả lời phỏng vấn và tư duy sự nghiệp ở tầm senior**.

---

## 📋 MỤC LỤC

| # | Module | Chủ đề | Nguồn liên quan |
|---|---|---|---|
| 01 | [Senior Mindset & Career Reality](01-Senior-Mindset-And-Career-Reality/README.md) | Tiêu chí thật phân biệt junior/mid/senior, "missing skills" CV không thể hiện | `01-Roadmaps/` |
| 02 | [System Design Interview Playbook](02-System-Design-Interview-Playbook/README.md) | Khung 5 bước trả lời mọi đề bài, bản đồ đề bài kinh điển → kiến thức cần dùng | Tổng hợp 3 cây kỹ thuật |
| 03 | [Technical Interview Strategy By Stack](03-Technical-Interview-Strategy-By-Stack/README.md) | Công thức trả lời câu hỏi lý thuyết/tradeoff/debug nghe "tầm senior" | `04-Interview-Prep/`, `interview_prep/` |
| 04 | [Behavioral & Salary Negotiation](04-Behavioral-And-Salary-Negotiation/README.md) | Khung STAR, câu chuyện lõi tái sử dụng, chiến lược đàm phán lương thật | `06-Exercises/CODE_EXERCISES.md` (phần HR) |

---

## 🗺️ Sơ đồ liên hệ — đây là điểm HỘI TỤ của toàn bộ hệ thống kiến thức

```
   DSA-Mastery          Backend-Mastery        Cloud-DevOps-Mastery
        │                      │                        │
        └──────────────────────┼────────────────────────┘
                                ▼
              02-System Design Interview Playbook
                   (tổng hợp cả 3 để giải 1 đề bài lớn)
                                │
                                ▼
              03-Technical Interview Strategy
                   (trình bày TỪNG mảnh kiến thức nghe thuyết phục)
                                │
                                ▼
              04-Behavioral & Salary Negotiation
                   (đóng gói thành câu chuyện + đàm phán thành công)
                                │
                                ▼
                    01-Senior Mindset
        (tư duy nền tảng xuyên suốt toàn bộ quá trình trên)
```

## Cách dùng
1. Đọc [`01-Senior-Mindset`](01-Senior-Mindset-And-Career-Reality/README.md) trước để hiểu đích đến (level senior thật sự là gì).
2. Ôn kiến thức kỹ thuật ở 3 cây `DSA-Mastery`/`Backend-Mastery`/`Cloud-DevOps-Mastery`.
3. Luyện tập system design theo [`02`](02-System-Design-Interview-Playbook/README.md) — đây là bài kiểm tra tổng hợp thật sự.
4. Luyện trình bày từng câu trả lời kỹ thuật đơn lẻ theo [`03`](03-Technical-Interview-Strategy-By-Stack/README.md).
5. Chuẩn bị phần con người + đàm phán theo [`04`](04-Behavioral-And-Salary-Negotiation/README.md).

## 🔗 Liên kết sang các cây khác
[`../DSA-Mastery/`](../DSA-Mastery/INDEX.md) · [`../Backend-Mastery/`](../Backend-Mastery/INDEX.md) · [`../Cloud-DevOps-Mastery/`](../Cloud-DevOps-Mastery/INDEX.md)
